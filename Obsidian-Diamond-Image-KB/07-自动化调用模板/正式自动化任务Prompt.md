...
7. 同一套 3 张图必须围绕同一个 product_dna。
8. 每套只抽少量变量，不要把所有词库都塞进提示词。
9. 人物画像围绕年轻网红、时装模特感达人、名媛、富婆、高消费力女性轮换；21-24 岁成人占 75%。
10. 产品可参考高奢珠宝设计语言，但最终提示词不得出现品牌名、Logo、同款、复刻。
11. 场景选定后，动作必须从兼容动作池抽取。
12. 全身图和半身图以人物、穿搭、气质、场景为主，产品只自然出现，不做硬展示。
13. 产品特写必须清楚展示钻石质感、切工、金属反射和合理结构，但要像真实生活局部裁切。
14. 多数图片使用普通真实拍摄角度，少量穿插时尚杂志化街拍、direct-flash、off-duty model 视角。
15. 正式生成每套输出 3 张独立图，不用三联图作为正式判断。
16. 生图后必须进入图片自动审核。
17. 合格图片进入素材库，不合格图片按规则重生或废弃。
18. 保存图片、product_dna、提示词、评分、索引、失败日志。
